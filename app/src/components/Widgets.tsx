...
import {
  faAngleDown, faAngleUp, faChartArea, faChartBar, faChartLine,
...
// --- Types ---
export interface ChoosePhotoWidgetProps {
  title: string;
  photo: string;
}
...
export interface ChartWidgetDataItem {
  id: number | string;
  label: string;
  value: number;
  color: string;
...
export interface SalesValueWidgetProps {
  title: string;
  value: number | string;
  percentage: number;
}
...
export const ChoosePhotoWidget: React.FC<ChoosePhotoWidgetProps> = ({ title, photo }) => (
...
export const CounterWidget: React.FC<CounterWidgetProps> = ({
  icon, iconColor, category, title, period, percentage
}) => {
...
export const CircleChartWidget: React.FC<CircleChartWidgetProps> = ({ title, data = [] }) => {
...
export const TeamMembersWidget: React.FC = () => {
  type TeamMemberProps = typeof teamMembers[number];
  const status = {
  ...
  const TeamMember: React.FC<TeamMemberProps> = ({ name, statusKey, image, icon, btnText }) => {
    const statusColor = status[statusKey]?.color ?? 'danger';
    const statusLabel = status[statusKey]?.label ?? 'Offline';
    return (
...
export const ProgressTrackWidget: React.FC = () => {
  ...
  const Progress: React.FC<ProgressProps> = ({ title, percentage, icon, color, last = false }) => {
...
export const SalesValueWidget: React.FC<SalesValueWidgetProps> = ({ title, value, percentage }) => {
  const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
  const percentageColor = percentage < 0 ? "text-danger" : "text-success";
  return (
...
export const SalesValueWidgetPhone: React.FC<SalesValueWidgetProps> = ({ title, value, percentage }) => {
  const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
  const percentageColor = percentage < 0 ? "text-danger" : "text-success";
  return (
    <Card className="bg-secondary-alt shadow-sm">
...
export const AcquisitionWidget: React.FC = () => (
  <Card border="light" className="shadow-sm">
    <Card.Body>
      <h5>Acquisition</h5>
